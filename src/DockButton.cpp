...
void DockButton::onDraw(cairo_t* cr)
{
	...
	if(mSOpened){
		if(mSFocus)
			cairo_set_source_rgba(cr, 0.75, 0.85, 0.95, 1);
		else
			cairo_set_source_rgba(cr, 0.7, 0.7, 0.7, 1);
		
		cairo_rectangle(cr, 0, h*0.9616, w, h);
		cairo_fill(cr);
	}
	...
	if(mSMany && (mSOpened || mSHover))
	{
		...
		cairo_pattern_add_color_stop_rgba(pat, 0.5, 0, 0, 0, 0.15);

		if(aBack > 0)
			cairo_rectangle(cr, x1, 0, w, h);
		else
			cairo_rectangle(cr, x1, h*0.9616, w, h);
